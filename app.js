...
const directoryPath = path.join(__dirname, "cbo");
// Read the JSX file
fs.readdir(directoryPath, (err, files) => {
  ...
  // Log the filenames
  files.forEach((file) => {
    const jsxCode = fs.readFileSync(path.join(directoryPath,file), "utf8");
      ...
      JSXText(path) {
        ...
        if (text) {
          ...
          const jsxExpressionContainer =
            t.jsxExpressionContainer(tCallExpression);
    ...
    fs.writeFileSync(path.join(directoryPath,file)+"x", output.code);
    console.log(file);
  });
});
